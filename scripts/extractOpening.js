...
for (let index = 0; index < surah_info.length; index++) {
  ...
  fs.writeFileSync(`src/data/surah-opening/${index+1}.ts`, `const opening = \`${opening}\`;
...
export default openingClosing;`, { encoding: 'utf-8' })
...
fs.writeFileSync(`src/data/surah-info-compact.ts`, `const surahInfo = ${JSON.stringify(newSurahInfo)};
export default surahInfo;`, { encoding: 'utf-8' })
