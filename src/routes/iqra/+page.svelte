...
<div class="px-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
	{#each IQRA_LEVELS as level (level.jilid)}
		...
		{@const percent =
			levelStats.total > 0 ? Math.round((levelStats.completed / levelStats.total) * 100) : 0}
		...
		{#if level.available}
			...
			>
				<div class="flex items-start justify-between gap-2">
					...
					{#if isDone}
						<span class="shrink-0 text-green-500 text-xl">✓</span>
					{/if}
				</div>
				...
				<div class="mt-1">
					...
					<div class="w-full h-1.5 rounded-full bg-primary overflow-hidden">
						<div
		...
		{:else}
			<div
				class="relative rounded-lg overflow-hidden shadow p-4 bg-secondary flex flex-col gap-2 opacity-60"
			>
				<div class="flex items-start justify-between gap-2">
					<div>
						<div
...
<div class="px-4 mt-8">
	{#if showResetConfirm}
		<div
			class="rounded-lg border border-red-300 bg-red-50 dark:bg-red-950/20 p-4 flex flex-col gap-3"
		>
			<p class="text-sm text-red-700 dark:text-red-300">{$t('iqra.resetConfirm')}</p>
			<div class="flex gap-2">
				<button
					onclick={handleReset}
					class="rounded-md bg-red-600 text-white px-3 py-1.5 text-sm hover:bg-red-700 transition"
				>
					{$t('common.confirm')}
				</button>
				<button
